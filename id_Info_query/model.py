...
def img2txt(binary_img_list):
    binary_img_list = list(
        binary_img_list) if type(binary_img_list) != list else binary_img_list
    img_sep = []
    ...
    return [('').join(img_ans[i * 5:(i + 1) * 5])
            for i, _ in enumerate(binary_img_list)]
